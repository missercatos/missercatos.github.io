// 基于 Canvas 的数码雨实现 - 优化内存版本（去除特殊字符）
...
function updateCurrentChars() {
    ...
    // 每15秒切换一次段落（让用户有足够时间阅读）
    setTimeout(updateCurrentChars, 15000);
}
// 计算列数 (根据字体大小)
const fontSize = 18;
let columns = 0;
...
function updateColumns() {
    ...
    for (let i = 0; i < columns; i++) {
        // 随机化初始位置，让雨滴看起来更自然
        drops[i] = Math.floor(Math.random() * -100); // 从画布上方开始
    }
}
// 绘制函数
function drawMatrix() {
    ...
    // 设置字体和颜色 (酸性青绿色)
    ctx.font = `${fontSize}px 'Courier New', monospace, 'Microsoft YaHei', 'SimSun'`;
    ...
    for (let i = 0; i < drops.length; i++) {
        ...
        if (charArray.length > 0) {
            ...
            if (y > canvas.height && Math.random() > 0.975) {
                drops[i] = Math.floor(Math.random() * -100); // 重置到画布上方随机位置
            }
            drops[i]++;
        }
    }
...
window.matrix = { 
    ...
    getAllSegments: () => textSegments // 新增：获取所有文本段落
};
// 页面加载完成后初始化
